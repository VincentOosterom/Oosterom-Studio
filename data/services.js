export const services = [
    {
        title: "Webdesign & UX",
        description:
            "Strategisch webdesign dat gebruikers richting conversie en groei stuurt.",
        bullets: [
            "UX research & wireframes",
            "High-end UI design",
            "Interactieve prototypes",
            "Conversiegericht ontwerp",
            "Schaalbare design systems"
        ],
        extra:
            "We ontwerpen niet alleen websites, maar interfaces die klaar zijn voor uitbreiding naar dashboards, portals en SaaS-producten.",
        icon: "🎨",
        link: "/diensten/webdesign"
    },
    {
        title: "Webdevelopment & Applicaties",
        description:
            "Snelle, veilige en schaalbare websites en webapplicaties gebouwd met moderne technologie.",
        bullets: [
            "React & moderne frontend",
            "Performance-first development",
            "SEO & toegankelijkheid",
            "Onderhoudbare code",
            "API-ready architectuur"
        ],
        extra:
            "Onze applicaties zijn gebouwd als fundament voor groei, van websites tot complete systemen zoals portals en SaaS-platformen.",
        icon: "💻",
        link: "/diensten/webdevelopment"
    },
    {
        title: "Mobile Apps",
        description:
            "Native mobiele apps voor iOS en Android, gebouwd met React Native.",
        bullets: [
            "React Native cross-platform development",
            "Offline-first architectuur",
            "Veilige authenticatie & data-encryptie",
            "App Store & Play Store publicatie",
            "Push-notificaties & integraties"
        ],
        extra:
            "Eén codebase voor zowel iOS als Android bespaart tijd en kosten, zonder in te leveren op prestaties of gebruikerservaring.",
        icon: "📱",
        link: "/diensten/mobile-apps"
    },
    {
        title: "SaaS & Digitale Systemen",
        description:
            "Van idee naar schaalbaar digitaal product met een sterke technische basis.",
        bullets: [
            "Productstructuur & MVP design",
            "Dashboard & gebruikersrollen",
            "Schaalbare architectuur",
            "Database design",
            "Groei-ready systemen"
        ],
        extra:
            "We helpen bij het vertalen van ideeën naar werkende producten die klaar zijn om te groeien en door te ontwikkelen.",
        icon: "🧩",
        link: "/diensten/saas"
    },
    {
        title: "Branding",
        description:
            "Merkidentiteit en visuele richting die je onderscheidt, van logo tot volledige huisstijl.",
        bullets: [
            "Merkstrategie & positionering",
            "Logo & huisstijlontwikkeling",
            "Merkrichtlijnen (brand guidelines)",
            "Visuele consistentie website & drukwerk",
            "Herkenbaarheid over alle kanalen"
        ],
        extra:
            "Een sterk merk maakt elk volgend design- of ontwikkelproject makkelijker, omdat de visuele en inhoudelijke richting al vaststaat.",
        icon: "🏷️",
        link: "/diensten/branding"
    },
    {
        title: "AI Agents & Automation",
        description:
            "AI-systemen die taken automatiseren, informatie verwerken en workflows ondersteunen.",
        bullets: [
            "AI chatbots met kennisbasis",
            "Workflow automatisering",
            "CRM & API integraties",
            "Slack / e-mail notificaties",
            "Controle & human-in-the-loop"
        ],
        extra:
            "Onze AI-agents zijn ontworpen met controle en betrouwbaarheid in gedachten, automatisering zonder dat je de regie verliest. Of kies voor jouw eigen agents.",
        icon: "🤖",
        link: "/diensten/ai-agents"
    },
    {
        title: "Software op Maat",
        description:
            "Maatwerk software die aansluit op jouw processen, gebouwd voor de lange termijn.",
        bullets: [
            "Procesanalyse & technisch advies",
            "Full stack development",
            "Koppeling met bestaande systemen",
            "Modulaire & uitbreidbare opbouw",
            "Documentatie & overdracht"
        ],
        extra:
            "Standaardsoftware past zelden perfect. Wij bouwen maatwerk dat aansluit op hoe jouw bedrijf daadwerkelijk werkt en eigendom van jou blijft.",
        icon: "⚙️",
        link: "/diensten/software-op-maat"
    },
    {
        title: "Klantportalen & CRM",
        description:
            "Eigen klantomgevingen en CRM-systemen gebouwd rondom hoe jij werkt.",
        bullets: [
            "Inlog & rechtenbeheer",
            "Dashboards per klant",
            "Realtime data & notificaties",
            "Koppeling met facturatie & mail",
            "Uitbreidbaar per fase"
        ],
        extra:
            "Een eigen portaal geeft klanten inzicht en jou minder e-mailverkeer. We bouwen modulair zodat het meegroeit met je bedrijf.",
        icon: "🗂️",
        link: "/diensten/klantportaal"
    },
    {
        title: "Webshops",
        description:
            "Shopify en maatwerk webshops die converteren, met een soepel bestelproces.",
        bullets: [
            "Shopify setup & thema-ontwikkeling",
            "Betaalproviders & verzendkoppelingen",
            "Voorraadbeheer & productdata",
            "Conversiegerichte checkout",
            "Schaalbaar bij groeiend assortiment"
        ],
        extra:
            "We bouwen webshops die niet alleen goed ogen, maar ook technisch soepel lopen bij piekmomenten zoals sales of feestdagen.",
        icon: "🛒",
        link: "/diensten/webshops"
    },
    {
        title: "SEO & Performance Optimalisatie",
        description:
            "Technische SEO en snelheidsoptimalisatie die je site écht hoger laat ranken.",
        bullets: [
            "Core Web Vitals optimalisatie",
            "Technische SEO-audit",
            "Structured data & schema markup",
            "Sitemap & crawlability",
            "Snelheids- en cache-optimalisatie"
        ],
        extra:
            "Een mooie website die traag laadt, verliest het alsnog van een snellere concurrent. Wij pakken snelheid en vindbaarheid samen aan, niet los van elkaar.",
        icon: "⚡",
        link: "/diensten/seo-performance"
    },
    {
        title: "Onderhoud & Support",
        description:
            "Betrouwbaar onderhoud en een vast aanspreekpunt voor stabiele, veilige systemen.",
        bullets: [
            "Updates & beveiligingspatches",
            "Uptime monitoring & back-ups",
            "Maandelijks urentegoed",
            "Prioriteit bij storingen",
            "Geen losse facturen per aanpassing"
        ],
        extra:
            "Met een onderhoudsabonnement weet je wat je krijgt en wat het kost. Geen gedoe, geen verrassingen. Gewoon een website die blijft werken.",
        icon: "🛠️",
        link: "/diensten/onderhoud-support"
    },
    {
        title: "Hosting & Infrastructuurbeheer",
        description:
            "Betrouwbare hosting en serverbeheer, zodat je site altijd bereikbaar blijft.",
        bullets: [
            "Uptime monitoring & alerting",
            "Automatische back-ups",
            "SSL-certificaten & beveiligingsupdates",
            "Schaalbare infrastructuur",
            "Snelle respons bij storingen"
        ],
        extra:
            "Hosting is meer dan een server aanzetten. Wij monitoren actief, patchen tijdig en grijpen in vóórdat een probleem bezoekers raakt.",
        icon: "🖥️",
        link: "/diensten/hosting-infrastructuur"
    },

];